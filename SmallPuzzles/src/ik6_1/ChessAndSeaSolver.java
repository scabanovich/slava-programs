...
//			if(chessWeight + chessWeights[f] > 18) continue;
...
//			System.out.println("");
	...
	boolean isSolution() {
		return usedShipsCount == ships.length && occupiedCells.count == field.getSize();
	}
	...
	class Knight extends Figure {
		protected void srch(int p) {
			...
			for (int i = 0; i < KNIGHT_JUMPS.length; i++) {
				int q = field.jump(p, KNIGHT_JUMPS[i][0], KNIGHT_JUMPS[i][1]);
				addWay(q, KNIGHT);
			}
		}
