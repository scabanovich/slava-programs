package slava.puzzle.shopping.analysis;
...
///					printSolution();
	...
	public void printTransitionStatistic() {
		for (int i = 0; i < nodes.length; i++) {
			...
			for (int j = 0; j < ts.length; j++) {
				...
				if(k == 0) continue;
				System.out.print(" " + getPresentation(ts[j]) + ":" + k);
			}
		}
		System.out.println("");
	}
	...
	private void printSolution() {
		for (int i = 0; i < t; i++)
			System.out.print(getPresentation(route[i]));
		System.out.println("");
	}
...
//		return "" + getPresentation(n.getId());
	...
	public static String getPresentation(Transition t) {
		return "" + t.getSource().getId() + "-" + t.getTarget().getId();
