...
public class SeaBattleNumberedLoader extends PuzzleLoader implements SeaBattleConstants {

	public SeaBattleNumberedLoader() {
		init();
	}
	...
	SeaBattleNumberedModel getSeaBattleModel() {
	...
	public void load() throws Exception {
		...
		SeaBattleNumberedPuzzle puzzle = getSeaBattleModel().getPuzzleInfo();
		...
		s = p.getProperty("numbers");
		ls = deserialize(s);
		for (int i = 0; i < field.getSize(); i++) puzzle.getNumbers()[i] = ls[i];
	...
	public void save() throws Exception {
		...
		p.setProperty("numbers", serialize(puzzle.getNumbers()));
	...
	public void saveHTML() throws Exception {
		...
		printCondition(b, field, hValues, vValues, numbers, data);
		b.print("<hr>");
		if(solution != null) printSolution(b, field, hValues, vValues, numbers, solution);
	...
	void printCondition(PrintWriter b, RectangularField field, int[] hValues, int[] vValues, int[] numbers, int[] data) {
		b.print("<html>\n");
		b.print("  <table>");
		for (int i = 0; i < field.getSize(); i++) {
			...
			if(numbers[i] >= 0) b.print(numbers[i]);
			b.print("</td>\n");
	...
	void printSolution(PrintWriter b, RectangularField field, int[] hValues, int[] vValues, int[] numbers, int[] solution) {
		b.print("<html>\n");
		b.print("  <table>");
		for (int i = 0; i < field.getSize(); i++) {
			if(field.getX(i) == 0) b.print("\n    <tr>\n");
			b.print("<td width=\"30\" height=\"30\" align=\"center\"");
			String bgc = solution[i] <= WATER ? "blue" : "6f6f6f";
			b.print(" bgcolor=\"" + bgc + "\"");
			b.print(">");
			if(numbers[i] >= 0) b.print(numbers[i]);
			b.print("</td>\n");
