...
public class SeaBattleNumberedComponent extends AbstractSeaBattleComponent implements SeaBattleConstants {
	private static final long serialVersionUID = 1L;
	SeaBattleNumberedPuzzle puzzle;
	SeaBattleNumberedPuzzleInput puzzleInput = new SeaBattleNumberedPuzzleInput();
	
	
	public SeaBattleNumberedComponent() {
		ML mouse = new ML();
	...
	public SeaBattleNumberedModel getSeaBattleModel() {
		return (SeaBattleNumberedModel)model;
	}
	...
	void paintCells(Graphics g) {
		...
		for (int i = 0; i < field.getSize(); i++) {
			...
			if(puzzle.getNumbers()[i] >= 0) {
				g.setColor(Color.BLACK);
				g.drawString("" + puzzle.getNumbers()[i], x + halfCellSize - 5, y + halfCellSize + 7);
			}
			if(!getSeaBattleModel().isShowingSolution() && i == puzzleInput.getPosition() && puzzleInput.getState() == SeaBattleNumberedPuzzleInput.STATE_B) {
	...
	void paintVValues(Graphics g) {
		for (int i = 0; i < field.getWidth(); i++) {
			...
			int y = margin;
			g.setColor(Color.WHITE);
			if(!getSeaBattleModel().isShowingSolution() && i == puzzleInput.getVPosition() && puzzleInput.getState() == SeaBattleNumberedPuzzleInput.STATE_V) {
				g.setColor(Color.YELLOW);
			}
	...
	void paintHValues(Graphics g) {
		for (int i = 0; i < field.getHeight(); i++) {
			...
			g.setColor(Color.WHITE);
			if(!getSeaBattleModel().isShowingSolution() && i == puzzleInput.getHPosition() && puzzleInput.getState() == SeaBattleNumberedPuzzleInput.STATE_H) {
				g.setColor(Color.YELLOW);
			}
			g.fillOval(x + 3, y + 3, cellSize - 5, cellSize - 5);
			if(v >= 0) {
	...
	void paintFleet(Graphics g) {
		...
		for (int i = 1; i < 6; i++) {
			...
			g.setColor(Color.WHITE);
			if(!getSeaBattleModel().isShowingSolution() && i == puzzleInput.getSPosition() && puzzleInput.getState() == SeaBattleNumberedPuzzleInput.STATE_S) {
				g.setColor(Color.YELLOW);
			}
